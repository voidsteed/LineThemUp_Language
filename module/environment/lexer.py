...
#accumulates a variable(including key word)
    ...
    def _isKeyword(self,token):
        if (token == "def" or token == "if" or token == "elif" or token == "else" or token == "while" or token == "for" or token == "not" or token == "in" or token == "range" or token == "return" or token == "print"):
            return True
        return False
