...
(define (one incrementer) ((increment zero) inc))
(define (two incrementer) ((increment one) inc))
(define (three incrementer) ((increment two) inc))
(define (four incrementer) ((increment three) inc))
(define (five incrementer) ((increment four) inc))
(define (six incrementer) ((increment five) inc))
(define (seven incrementer) ((increment six) inc))
(define (eight incrementer) ((increment seven) inc))
(define (nine incrementer) ((increment eight) inc))
...
(define (powerSet set)
        (define (powerRecur a)
                (if (null? a) (list '())
                        (let ((p(powerRecur (cdr a))))
                                (define powerRecur (append (map (lambda(x) (cons (car a) x))p) p)))))
        (define unfilSet (powerRecur set))
        (define (filterRecur filSet lnth)
                (cond
                        ((= lnth (length unfilSet))
                                (append filSet (keep helper unfilSet lnth)))
                        (else
                                (filterRecur (append filSet (keep helper unfilSet lnth)) (+ lnth 1)))))
        (cons '() (filterRecur nil 0)))


(define (keep p? items lngth)
        (cond
                ((null? items) nil)
                ((p? (car items) lngth)
                        (cons (car items) (keep p? (cdr items) lngth)))
                (else (keep p? (cdr items) lngth))))
...
(define (Rectangular num imag)
        (define (type)
                2)
        (define (add num2)
                (Rectangular (+ num (car((num2 'value)))) (+ imag (cdr ((num2 'value))))))
        (define (promote)
...
(define (Polar mag theta)
        ...
        (define (add num2)
                ...
                (define rect2 ((num2 'convert)))
                ((((rect1 'add) rect2) 'convert)))
        (define (promote)
                (Polar mag theta))
        (define (demote)
                (((convert) 'demote)))
...
(redefine (+ num1 num2)
        (if (or (integer? num1) (real? num1) (not (eq? (car num1) 'object)))
                (begin
                        ;Our type hierarchy:
                        ;       polar = 3
                        ;       rect = 2
                ...
                (begin
                        (cond
                                ((or (and (= ((num1 'type)) 2) (= ((num2 'type)) 3)) (and (= ((num1 'type)) 3) (= ((num2 'type)) 2)))
                                        (+ num1 ((num2 'convert))))
                                ((< ((num1 'type)) ((num2 'type)))
                                        (cond
                                                ((or (= ((num2 'type)) 2) (= ((num2 'type)) 3))
                                                        (+ num2 ((num1 'promote))))
                                                (else
                                                        (+ ((num1 'promote)) num2))))
                                ((> ((num1 'type)) ((num2 'type)))
...
;*****************************Problem10*****************************
(put '(Integer real) (lambda (x) ((x 'promote))))
(put '(Integer rectangular) (lambda (x) ((((x 'promote)) 'promote))))
(put '(Integer polar) (lambda (x) ((((((x 'promote)) 'promote)) 'convert))))
(put '(Real integer) (lambda (x) ((x 'demote))))
(put '(Real rectangular) (lambda (x) ((x 'promote))))
(put '(Real polar) (lambda (x) ((((x 'promote)) 'convert))))
(put '(Rectangular integer) (lambda (x) ((((x 'demote)) 'demote))))
(put '(Rectangular real) (lambda (x) ((x 'demote))))
(put '(Rectangular polar) (lambda (x) ((x 'convert))))
(put '(Polar integer) (lambda (x) ((((x 'demote)) 'demote))))
(put '(Polar real) (lambda (x) ((x 'demote))))
(put '(Polar rectangular) (lambda (x) ((x 'convert))))
...
zero ultimately returns (incrementer ((zero incrementer) base)). Since the 
...
(define (run5)
	...
	(println "The matrix-*-vector product of the matrix ((1 0) (-1 -3) (2 1)) and the \nvector (2 1 0) is " (matrix-*-vector (list (list 1 0) (list -1 -3) (list 2 1)) (list 2 1 0)) " [Should be (1 -3)]\n")
	(println "The matrix-*-matrix product of ((0 -4)(4 -3)(-2 0)) and ((0 1 2)(1 -1 3)) is \n" (matrix-*-matrix (list (list 0 -4) (list 4 -3) (list -2 0))(list (list 0 1 2) (list 1 -1 3))) " [Should be ((0 -3)(-10 -1))]\n")
	(println "transposing matrix ((1 5 9)(22 44 88)(33 76 0)) results in \n" (transpose '((1 5 9) (22 44 88) (33 76 0))) " [Should be ((1 22 33)(5 44 76)(9 88 0))]\n")
	(println))
...
(define (run9)
	...
	(define pol1 (Polar 5 3.14))
	(define pol2 (Polar 6.2 6))
	(println "The sum of 22 and 999 is " (((+ (Integer 22) (Integer 999)) 'toString)) " [Should be 1021]\n")
	(println "The sum of 1.3 and 6.2 is " (((+ (Real 1.3) (Real 6.2)) 'toString)) " [Should be 7.5]\n")
	(println "The sum of the rectangles " ((rect1 'toString)) " and " ((rect2 'toString)) " is " (((+ rect1 rect2) 'toString)) " [Should be " (((Rectangular 7 13.3) 'toString)) "]\n")
	(println "The sum of the polars " ((pol1 'toString)) " and " ((pol2 'toString)) " is " (((+ pol1 pol2) 'toString)) " [Should be " (((Polar 1.97026064 -1.065891804) 'toString)) "]\n")
	(println "The sum of the rectangle " ((rect1 'toString)) " and the polar " ((pol2 'toString)) " is " (((+ rect1 pol2) 'toString)) " [Should be " (((Rectangular 6.953055777 1.267623911) 'toString)) "]\n")
	(println "The sum of the polar " ((pol1 'toString)) " and the rectangle " ((rect2 'toString)) " is " (((+ pol1 rect2) 'toString)) " [Should be " (((Polar 10.35635647 1.474085976) 'toString)) "]\n")
	(println "The sum of 7 and the polar " ((pol2 'toString)) " is " (((+ (Integer 7) pol2) 'toString)) " [Should be " (((Polar 13.06838861 -0.1329536841) 'toString)) "]\n")
	(println))

(define (run10)
	(println "Problem 10:")
	(define i (Integer 7))
	(define r (Real 5.789234))
	(define rect (Rectangular 2 2))
	(define n (Polar 1 3.14))
	(println "(((coerce (Integer 7) 'polar) 'toString)) returns " (((coerce i 'polar) 'toString)) "\n")
	(println "(((coerce (Rectangular 2 2) 'integer) 'toString)) returns " (((coerce rect 'integer) 'toString)) "\n")
	(println "(((coerce (Real 5.789234) 'rectangular) 'toString)) returns " (((coerce r 'rectangular) 'toString))"\n")
	(println "(((coerce (Polar 1 3.14) 'real) 'toString)) returns " (((coerce n 'real) 'toString))"\n")
	(println))
